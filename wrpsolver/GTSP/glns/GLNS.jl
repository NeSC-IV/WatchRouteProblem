...
"""
function solver(num_vertices, num_sets, sets, membership, dist; args...)
	###### Read problem data and solver settings ########
	sets = convert(Array{Any, 1},sets)
	param = parameter_settings(num_vertices, num_sets, sets, args)
	#####################################################
	init_time = time()
	...
	timer = (time_ns() - start_time)/1.0e9
	return lowest.cost,lowest.tour
end
end
