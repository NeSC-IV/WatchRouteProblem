...
class ResEncoder(nn.Module):
    def __init__(
            self,obs_shape,hidden_size_list,activation,kernel_size,stride,padding,norm_type
    ) -> None:
        block = BasicBlock
        num_classes = hidden_size_list[-1]
        layers = [2,2,2,2]
        super(ResEncoder, self).__init__()
        self.inplanes = 64

        m = OrderedDict()
        m['conv1'] = nn.Conv2d(obs_shape, 64, kernel_size=7, stride=2, padding=3, bias=False)
        m['bn1'] = nn.BatchNorm2d(64)
        m['relu1'] = nn.ReLU(inplace=True)
        m['maxpool'] = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.group1= nn.Sequential(m)
