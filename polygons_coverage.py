...
def SelectMaxPolygon(polygons):
    ...
    if (type(polygons) == type(shapely.Polygon())):
        MaxPolygon = polygons
    else:
        for p in list(polygons.geoms):
            if p.area > MaxPolygon.area:
                MaxPolygon = p
    return MaxPolygon
...
def MaximallyCoveringConvexSubset(maxUncoveredPolygon, watcher, d):  # MCCS

    visiblePolygon = FindVisibleRegion(
        maxUncoveredPolygon, watcher, d)  # d为可视距离
    ...
    for i in range(numOfReflexPoints):
        ...
        polygon = max(ePolygon1, ePolygon2, tPolygon, sPolygon, key=lambda inptPolygon: (
            maxUncoveredPolygon.intersection(inptPolygon)).area)
...
def PolygonCover(polygon, d, coverage, iterations=10):
    ...
    while ((unCoverPolygon.area / polygon.area) > coverage):
        ...
        R0 = MaximallyCoveringConvexSubset(maxUncoveredPolygon, point, d)
        bestR = R0
